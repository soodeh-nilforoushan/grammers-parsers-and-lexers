...
//Variables used only in this module.
 ...
 int item_found(char *s) {	//Check  mikone item dar state.
 	int i;

 	for(i = 0; i < closure_item_index; i++) {
 		if(!strcmp(s, items[state_index][i]))	//If  strings match bood.
 			return 1;
 	}
 ...
 int Goto1(char s, char temp[][100]) {	//d. ra peida mikone
 	int i, j;
 	int n = 0;
 	char t, temp2[100];
 ...
 int state_found(char *s) {	//check mikoe age satate yeksan vojood dashte  bashe
 	int i;
